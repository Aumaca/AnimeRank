...
import { MessageState, Country, FormDataState, FormErrorState } from "../interfaces.tsx"
...
const Register = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post(`${import.meta.env.VITE_API_URL}/auth/register`, newFormData)
      .then(response => {
      ...
      .catch(error => {
        if (error.response.status === 400) {
          ...
          setFormError(previousState => ({ ...previousState, [field]:  message}))
        }
  ...
  return (
    <>
      ...
      <div className="auth">
        <div className="container">
          <form onSubmit={handleSubmit} encType="multipart/form-data">
            ...
            <div className="field">
              <label>Country</label>
              <select name="country" onChange={handleChange} value={formData.country} className={`${formError.country ? "error" : ""}`}>
            ...
            <div className="field">
              ...
              <div {...getRootProps({className: `dropzone ${formError.password ? "error" : ""}`})}>
                <input {...getInputProps()} />
            ...
            <Link to="/login" className="redirect">Or login <FontAwesomeIcon icon={faArrowRight} /></Link>
          </form>
        </div>
      </div>
      <Modal isOpen={isModalOpen} backgroundColor={"#50C878"}>
      ...
      <Message closeMessage={closeMessage} isOpen={messageState.isOpen} title={messageState.title} backgroundColor={messageState.backgroundColor}>{messageState.children}</Message>
    </>
  )
}
